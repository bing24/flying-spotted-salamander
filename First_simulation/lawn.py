...
class ANIMATION(object):
    ...
    def addGlider(self, number):
        ...
        for i in range(0, number):
            ...
            print "For glider #", i, "deploy time, travel time and stop times are", self.gliders[i].deploy_time, self.gliders[i].travel_time, self.gliders[i].stop_time
            print "Glider meets its target @", target_x, target_y
...
anim = animation.FuncAnimation(fig, anime.animate, init_func=anime.setup,
                               frames=8000, interval=2, repeat=False)
